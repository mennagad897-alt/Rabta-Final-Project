...
interface CreatePostModalProps {
  ...
  onPostSuccess: (postData: any) => void; // 💡 هنبعت الداتا بتاعت البوست هنا
}
...
export const CreatePostModal: React.FC<CreatePostModalProps> = ({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // 💡 حفظنا النتيجة في متغير عشان نبعتها
      ...
      // تنظيف الفورم وقفل المودال بعد النجاح وبعتنا بيانات البوست للشات
      if (response.data?.data?.post) {
        onPostSuccess(response.data.data.post);
      }
      setContent("");
      setSelectedFiles([]);
      onClose();
      ...
      <div className="relative w-full max-w-lg bg-[#1E1E1E] rounded-2xl border border-white/10 shadow-2xl flex flex-col animate-in fade-in zoom-in-95 duration-200">
          ...
          {selectedFiles.length > 0 && (
            <div className="flex flex-wrap gap-2 mt-2">
              {selectedFiles.map((file, idx) => (
                <div
                  key={idx}
                  className="flex items-center gap-2 bg-[#262626] px-3 py-1.5 rounded-lg border border-white/10"
                >
        ...
        <div className="p-4 flex items-center justify-between border-t border-white/5">
          ...
          >
            ...
            {!isSubmitting && (
              <span className="material-icons-round text-sm">double_arrow</span>
            )}
          </button>
        </div>
      </div>
