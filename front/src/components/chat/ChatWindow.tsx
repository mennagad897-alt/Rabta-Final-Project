...
const DirectChatSharedMedia = ({ chatId }: { chatId: string }) => {
  const [activeShared, setActiveShared] = useState<'media' | 'docs' | 'photos' | null>(null);
  ...
  const fetchShared = async (category: 'media' | 'docs' | 'photos') => {
    ...
    try {
      const res = await axiosInstance.get(`/chats/${chatId}/shared`, { params: { category } });
          ...
          {isGroup ? (
            ...
            <div className="flex flex-col h-full">
              <div className="p-4 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between shrink-0">
                <h2 className="font-bold text-[#171717] dark:text-[#F5F5F5] text-base">Contact Info</h2>
                <button
                  type="button"
