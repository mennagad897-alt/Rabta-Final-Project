...
const DirectChatSharedMedia = ({ chatId }: { chatId: string }) => {
  const [activeShared, setActiveShared] = useState<
    "media" | "docs" | "photos" | null
  >(null);
  const [sharedItems, setSharedItems] = useState<any[]>([]);
  const [loadingShared, setLoadingShared] = useState(false);

  const fetchShared = async (category: "media" | "docs" | "photos") => {
                  ...
                  <li key={item.id} className="relative group">
                    {isImg ? (
                      <a
                        ...
                        className="block aspect-square rounded-lg overflow-hidden bg-gray-100 dark:bg-gray-800"
                      >
                        <img
                          src={href}
                          alt=""
...
export const ChatWindow: React.FC<ChatWindowProps> = ({
  ...
  const { isOnline: isUserOnline } = useOnlineUsers();
  const isPersonOnline = !isGroup && receiverId ? isUserOnline(receiverId) : isOnline;
  ...
  useEffect(() => {
    ...
    const handleReceiveMessage = (incoming: unknown) => {
      ...
      fetch("/api/notifications/settings", {
        ...
        .then((settings) => {
          const allowed =
            (isGroup && settings?.communityMentions !== false) ||
            (!isGroup && settings?.chatMessages !== false);
          if (allowed) {
      ...
      const formatted: MessageType = {
        ...
        fileSize: incomingMsg.attachments?.[0]?.fileSize
          ? (incomingMsg.attachments[0].fileSize / 1024 / 1024).toFixed(2) +
            " MB"
          : undefined,
            ...
            {!isGroup && (
              <div className="relative w-10 h-10 shrink-0">
                {contactProfile?.avatar ? (
                  ...
                  <div className="w-full h-full rounded-full bg-[#7C3AED]/10 text-[#7C3AED] flex items-center justify-center font-bold">
                    {(chatName || "U")[0].toUpperCase()}
                  </div>
                )}
                {isPersonOnline && showOnlineStatus && (
                  <span className="w-3 h-3 bg-green-400 rounded-full absolute bottom-0 right-0 ring-2 ring-white dark:ring-[#262626] animate-pulse"></span>
                )}
              </div>
            )}
                                    ...
                                    if (isImage) {
                                      ...
                                      const fullUrl = rawUrl
                                        ? rawUrl.startsWith("http")
                                          ? rawUrl
                                          : `${import.meta.env.VITE_API_URL?.replace("/api/v1", "") || "http://localhost:5000"}${rawUrl}`
                                        : "";
                                      return (
                        ...
                        {msg.type === "audio" ? (
                          <div className="flex flex-col">
                            <audio
                              ...
                              src={`${import.meta.env.VITE_API_URL?.replace("/api/v1", "") || "http://localhost:5000"}${msg?.content || msg?.fileUrl}`}
                          ...
                          (() => {
                            ...
                            const fullUrl = fileUrl
                              ? `${import.meta.env.VITE_API_URL?.replace("/api/v1", "") || "http://localhost:5000"}${fileUrl}`
                              : "#";
                            const isImage =
                              msg?.fileName?.match(
                                /\.(jpeg|jpg|gif|png|webp)$/i,
                      ...
                      {showAiPopup && (
                        <div className="absolute bottom-[calc(100%+10px)] left-4 right-4 md:left-auto md:right-auto md:w-80 bg-white dark:bg-[#262626] rounded-2xl shadow-2xl border border-gray-100 dark:border-white/5 overflow-hidden flex flex-col z-50">
                          ...
                          <div className="h-48 bg-[#FAFAFA] dark:bg-[#171717] p-4 text-sm text-gray-500 italic overflow-y-auto">
                            How can I help you with your conversation with{" "}
                            {chatName}?
                          </div>
                        </div>
                      )}
          ...
          {isGroup ? (
            ...
            <div className="flex flex-col h-full">
              <div className="p-4 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between shrink-0">
                <h2 className="font-bold text-[#171717] dark:text-[#F5F5F5] text-base">
                  Contact Info
                </h2>
                <button
                  type="button"
              ...
              <div className="flex-1 overflow-y-auto hide-scrollbar p-5 space-y-6">
                <div className="flex flex-col items-center text-center">
                  ...
                  <p className="text-sm text-[#7C3AED] dark:text-[#8B5CF6] font-semibold mt-0.5">
                    Contact
                  </p>
