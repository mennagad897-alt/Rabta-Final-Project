...
import { smartSearchPromptTemplate } from "./prompts.ai"; 
...
export const askGlobalKnowledge = async (question: string) => {
  return "This function is handled natively by specific route endpoints.";
};
...
export const semanticSearchMessages = async (query: string, userId: string, chatId: string, currentUserName: string) => {
  ...
  const contextPromises = searchResults.map(async (doc) => {
    ...
    const time = doc.metadata?.timestamp 
      ? new Date(doc.metadata.timestamp).toLocaleTimeString('ar-EG', { hour: '2-digit', minute: '2-digit' }) 
      : "غير محدد";
